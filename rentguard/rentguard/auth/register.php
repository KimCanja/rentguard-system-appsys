    ...
    if (empty($name) || empty($email) || empty($password) || empty($confirm_password)) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            try {
                ...
                $stmt->execute([$name, $email, $encrypted_password, $role]);
...
<body>
        ...
        <form method="POST" id="registerForm" autocomplete="off">
            ...
            <div class="mb-3">
                <label class="form-label">
                    Password 
                    <i class="fas fa-info-circle info-icon" title="Password must be at least 12 characters and include uppercase, lowercase, number, and special character"></i>
                </label>
    ...
    <script>
        ...
        function calculatePasswordStrength(password) {
            ...
            if (password.match(/[A-Z]/)) criteria++;
            if (password.match(/[a-z]/)) criteria++;
            if (password.match(/[0-9]/)) criteria++;
        ...
        function validateForm() {
            ...
            const isValid = requirements.isValid && doPasswordsMatch && password.length > 0;
            ...
            inputs.forEach(input => {
                if (input.value && input.type !== 'submit' && input.type !== 'button') {
                    if (input.type === 'email' || input.type === 'text') {
                        // Don't clear if there's an error (to show what was entered)
                        <?php if (empty($error)): ?>
                        input.value = '';
                        <?php endif; ?>
                    }
