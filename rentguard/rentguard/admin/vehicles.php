        ...
        try {
            if ($vehicle_id) {
                ...
                if ($photo_url) {
                    $stmt = $pdo->prepare("
                        UPDATE vehicles 
                        SET model = ?, plate_number = ?, year = ?, type = ?, status = ?, 
                            current_mileage = ?, price_per_day = ?, photo_url = ?
                        WHERE vehicle_id = ?
                    ");
                    $stmt->execute([$model, $plate_number, $year, $type, $status, $current_mileage, $price_per_day, $photo_url, $vehicle_id]);
                } else {
                    $stmt = $pdo->prepare("
                        UPDATE vehicles 
                        SET model = ?, plate_number = ?, year = ?, type = ?, status = ?, 
                            current_mileage = ?, price_per_day = ?
                        WHERE vehicle_id = ?
                    ");
                    $stmt->execute([$model, $plate_number, $year, $type, $status, $current_mileage, $price_per_day, $vehicle_id]);
                }
                $success = 'Vehicle updated successfully!';
            } else {
                // Insert new vehicle
                $stmt = $pdo->prepare("
                    INSERT INTO vehicles (model, plate_number, year, type, status, current_mileage, price_per_day, photo_url)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([$model, $plate_number, $year, $type, $status, $current_mileage, $price_per_day, $photo_url]);
